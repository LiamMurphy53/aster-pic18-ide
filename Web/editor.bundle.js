(() => {
  ...
  var ContentBuilder = class _ContentBuilder {
    ...
    static build(text, from, to, decorations3, dynamicDecorationMap) {
  ...
  function findCompositionRange(view2, changes, headPos) {
    ...
    for (let parent = textNode.parentNode; ; parent = parent.parentNode) {
      ...
      if (parentView instanceof MarkView)
        marks3.push({ node: parent, deco: parentView.mark });
      else if (parentView instanceof LineView || parent.nodeName == "DIV" && parent.parentNode == view2.contentDOM)
        return { range, text: textNode, marks: marks3, line: parent };
      else if (parent != view2.contentDOM)
        marks3.push({ node: parent, deco: new MarkDecoration({
          inclusive: true,
  ...
  var selectionLayer = /* @__PURE__ */ layer({
    above: false,
    markers(view2) {
      return view2.state.selection.ranges.map((r) => r.empty ? [] : RectangleMarker.forRange(view2, "cm-selectionBackground", r)).reduce((a2, b) => a2.concat(b));
    },
    update(update, dom) {
  ...
  var activeLineGutterHighlighter = /* @__PURE__ */ gutterLineClass.compute(["selection"], (state) => {
    ...
    for (let range of state.selection.ranges) {
      ...
      if (linePos > last) {
        last = linePos;
        marks3.push(activeLineGutterMarker.range(linePos));
      }
    }
    return RangeSet.of(marks3);
  });
  ...
  var bracketMatchingState = /* @__PURE__ */ StateField.define({
    ...
    update(deco, tr) {
      ...
      for (let range of tr.state.selection.ranges) {
        ...
        if (match)
          decorations3 = decorations3.concat(config2.renderMatch(match, tr.state));
      }
  ...
  var asm = StreamLanguage.define({ startState: () => ({ comment: false }), token(stream, state) {
    ...
    if (stream.match(/^[A-Za-z_.$][\w.$]*/)) {
      const t2 = stream.current().toUpperCase();
      if (stream.peek() === ":") return "labelName";
      if (instructions.includes(t2)) return "keyword";
      if (directives.includes(t2)) return "meta";
      if (/^(WREG|STATUS|BSR|PORT[A-J]|LAT[A-J]|TRIS[A-J]|[AWFB])$/.test(t2)) return "variableName";
      return "name";
    }
    stream.next();
    return null;
  ...
  var dot = new Dot();
  var setMarks = StateEffect.define();
  var marks2 = StateField.define({ create: () => ({ breaks: [], execution: 0, errors: [] }), update(v, tr) {
    for (const e of tr.effects) if (e.is(setMarks)) return e.value;
    return v;
  } });
  var decorations2 = EditorView.decorations.compute([marks2], (state) => {
    const m = state.field(marks2), all = [];
    if (m.execution > 0 && m.execution <= state.doc.lines) all.push(Decoration.line({ class: "cm-execution" }).range(state.doc.line(m.execution).from));
    for (const n of m.errors) if (n > 0 && n <= state.doc.lines) all.push(Decoration.line({ class: "cm-problem-line" }).range(state.doc.line(n).from));
  ...
  function makeState(content2, path, readOnly2) {
    return EditorState.create({ doc: content2, extensions: [gutter({ class: "cm-breakpoint-gutter", renderEmptyElements: true, lineMarker: (v, line) => v.state.field(marks2).breaks.includes(v.state.doc.lineAt(line.from).number) ? dot : null, lineMarkerChange: (update) => update.startState.field(marks2) !== update.state.field(marks2), domEventHandlers: { mousedown: breakpointClick } }), lineNumbers({ domEventHandlers: { mousedown: breakpointClick } }), highlightActiveLineGutter(), history(), drawSelection(), rectangularSelection(), crosshairCursor(), highlightActiveLine(), highlightSelectionMatches(), indentOnInput(), bracketMatching(), closeBrackets(), foldGutter(), language2.of(/\.(c|h|cpp)$/.test(path) ? cpp() : asm), theme2, syntaxHighlighting(syntax), readonly.of(EditorState.readOnly.of(readOnly2)), autocompletion({ override: [completions] }), keymap.of([{ key: "Mod-s", run: () => {
      ...
      if (update.selectionSet || update.docChanged) {
        ...
        callbacks.cursor?.(line.number, pos - line.from + 1);
      }
    })] });
  }
  window.AsterEditor = {
    init(parent, options) {
      callbacks = options;
      view = new EditorView({ parent, state: makeState("", "", true) });
    },
    setDevice(regs) {
      device = regs;
    },
    open(path, content2, readOnly2 = false) {
      if (activePath) states.set(activePath, view.state);
      activePath = path;
      view.setState(states.get(path) || makeState(content2, path, readOnly2));
      view.dispatch({ effects: readonly.reconfigure(EditorState.readOnly.of(readOnly2)) });
    },
    close(path) {
      states.delete(path);
    ...
    go(line, column = 1) {
      const l = view.state.doc.line(Math.max(1, Math.min(line, view.state.doc.lines))), pos = Math.min(l.to, l.from + column - 1);
      view.dispatch({ selection: { anchor: pos }, effects: EditorView.scrollIntoView(pos, { y: "center" }) });
      view.focus();
    },
    marks(breaks = [], execution = 0, errors = []) {
      view.dispatch({ effects: setMarks.of({ breaks, execution, errors }) });
    },
    readOnly(value) {
